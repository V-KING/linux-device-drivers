/* 参考linux-3.4.2\drivers\net\ethernet\davicom\dm9000.c */
//DM9000 驱动解析参考 :http://blog.csdn.net/ypoflyer/article/details/6209922
 ...
 */
//0x0000~0x0BFF是TX Buffer， 地址0x0C00~0x3FFF是RX Buffer
static int
dm9000_start_xmit(struct sk_buff *skb, struct net_device *dev)
{
	...
	/* Move data to DM9000 TX RAM 写数据到DM9000 TX buffer */
	...
	/*如果发送的是第二个数据包（表明队列中此时有包发送），则将其加入队列中：将skb->len和skb->ip_summed
	 *（控制校验操作）赋值给board_info_t中有关队列的相关成员。调用函数 netif_stop_queue(dev)，
	 *通知内核现在queue已满，不能再将发送数据传到队列中，注：第二个包的发送将在tx_done中实现。*/  
	else {
		/* Second packet */
...
//dm9000_driver中定义 .pm = &dm9000_drv_pm_ops
...
static int __devexit dm9000_drv_remove(struct platform_device *pdev)
{
	...
	dm9000_release_board(pdev, net_priv(ndev));
	free_netdev(ndev);
	return 0;
}


//板极初始化文件中会有static struct platform_device .name = DM9000
